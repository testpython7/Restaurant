...
class RestaurantsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Restaurant
        fields = ['name','like','description','address','phone_number','opening_time','closing_time','menucount', 'manuiteams']
...
class UserputLikeResSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserLikeRetaurant
        fields = ['restaurant','likes']
    def validate(self, validated_data):
        ...
        if likes == True: 
            if likes == True and existing_data:
                ...
                p.delete()
                return validated_data
            elif likes == True and not existing_data:
                raise serializers.ValidationError("not liked before")
        ...
        if likes == False: 
            if likes == False and existing_da:
                ...
                p.delete()
                return validated_data
            elif likes == False and not existing_da:
                raise serializers.ValidationError("not dis-liked before")
